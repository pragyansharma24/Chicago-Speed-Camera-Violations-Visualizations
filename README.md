...
**Address:** Address of the location of the speed enforcement camera(s). There may be more than one camera at each address. </br>
...
* Line charts are often the preferred chart to show trend in the data and Blue is standard chart color.
* Weekly chart gives a clearer picture without losing granularity of the data. Although similar trend can be seen in annual, monthly or daily charts were but weekly appears to more readable, less distracting, and conveys the desired message. Also, the daily charts has a lot of noise. 
*  why not moving average- the graph is much smoother when we use moving average . The current graph also shows that even though the tred is downward there is a lot variation(seasonality) in the violations per week.
* Considering sum of violations over average was done because it is directly associated with the amount revenue is generated. 
* Labeling few high and low points for increases readabilty and relatabilty 
* why trendline As we are not predicting anything and confidence interval is not required. it is not meant for mathematician. 

In the previous version, this chart had red-blue colormap. However, now I understand that colors are actually not required and perhaps distracts the reader. Using a single color now for the time series plot. Adding confidence interval, what does it explains? Caption should be to the point. Improve titles. Explain solid line, dash line etc. 

Clear message to the Mayor:
1. Number of violations are on decline. Trendline shows reduction by 4.42 every day.
2. Future revenue will go down, the city of chicago should consider this for future policy making. 
3. Possibly efforts to make violation system have made the system more efficient, which is necessary for the success of this project. 
